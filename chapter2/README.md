...
正しい形式意味論はきちんと定義された数学的オブジェクトに変換することでプログラムの核心に迫る。
今回やったのは形式的ではないので、そこは注意すること。

表示的意味論は、別の言語への変換だから、コンパイラを実装するのに似ている。
これによって別々の言語で作られたプログラムの比較ができたりする。

現実世界では、Javaプログラムがjavacでバイトコードにコンパイルされ、バイトコードはJava仮想マシンによってx86インストラクションに実行時コンパイルされ、そしてx86インストラクションはCPUによって、コアで実行するためにRISCライクのマイクロインストラクションにデコードされ… と、実際に行われている。


#### その他のスタイル

- スモールステップ意味論
  - 構造的操作的意味論 (structural operational semantics)
...
抽象機械の状態を表明(assertion)することで意味を記述してゆく。
事前条件(precondition)が真ならば、文の実行後に事後条件(postcondition)が真になる。


参考： 公理的意味論に似た例として、RubySpecプロジェクトがある。  
http://rubyspec.org

RSpecの表明を用い、Ruby組み込みの言語構成要素、コアおよび標準ライブラリの振る舞いを記述している。
